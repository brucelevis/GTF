...
namespace ImGui
{
    struct DockContext
    {
        ...
        bool tabbar(Dock& dock, bool close_button)
        {
            ...
            PushStyleColor(ImGuiCol_ChildWindowBg, ImVec4(0, 0, 0, 0));
        ...
        bool isAnyFloatingDockDragged()
        {
            for (int i = 0; i < m_docks.size(); ++i)
            {
                Dock& dock = *m_docks[i];
                if(dock.status == Status_Dragged) return true;
            }
        ...
        */
        ...
        void reset(MPDockData * allDocks = nullptr, unsigned int numElements = 12)
        {
            for (int i = 0; i < m_docks.size(); ++i)
            {
                m_docks[i]->~Dock();
            ...
            MPDockData defaultDocks[] =
            {
                /*{ 0, "Color Picker", 1595, 33, 325, 397, 0, 1, 1, -1, -1, -1, -1, 9},
                { 1, "Layers", 1595, 430, 325, 683, 0, 1, 1, -1, -1, -1, -1, 9},
                { 2, "Texture Gallery", 276, 740, 1319, 373, 0, 1, 1, 4, -1, -1, -1, 6},
                { 3, "Viewport Settings", 0, 33, 276, 1080, 0, 1, 1, -1, -1, -1, -1, 7},
                { 4, "Shader Editor", 276, 740, 1319, 373, 0, 0, 1, -1, 2, -1, -1, 6},
                { 5, "3D View", 276, 33, 1319, 707, 0, 1, 1, -1, -1, -1, -1, 6},
                { 6, "", 276, 33, 1319, 1080, 0, 1, 0, -1, -1, 5, 4, 7},
                { 7, "", 0, 33, 1595, 1080, 0, 1, 0, -1, -1, 3, 6, 8},
                { 8, "", 0, 33, 1920, 1080, 0, 1, 0, -1, -1, 7, 9, -1},
                { 9, "", 1595, 33, 325, 1080, 0, 1, 0, -1, -1, 0, 1, 8}*/
                
                { 0, "Color Picker", 1595, 680, 325, 400, 0, 1, 1, -1, -1, -1, -1, 11},
                { 1, "Layers", 1595, 33, 325, 647, 0, 1, 1, -1, -1, -1, -1, 11},
                { 2, "Texture Gallery", 226, 718, 1369, 362, 0, 1, 1, 4, -1, -1, -1, 6},
                { 3, "Viewport Settings", 0, 33, 226, 1047, 0, 1, 1, -1, -1, -1, -1, 7},
                { 4, "Shader Editor", 226, 718, 1369, 362, 0, 0, 1, -1, 2, -1, -1, 6},
                { 5, "3D View", 226, 33, 673, 685, 0, 1, 1, -1, -1, -1, -1, 10},
                { 6, "", 226, 33, 1369, 1047, 0, 1, 0, -1, -1, 10, 4, 7},
                { 7, "", 0, 33, 1595, 1047, 0, 1, 0, -1, -1, 3, 6, 8},
                { 8, "", 0, 33, 1920, 1047, 0, 1, 0, -1, -1, 7, 11, -1},
                { 9, "2D Viewport", 899, 33, 696, 685, 0, 1, 1, -1, -1, -1, -1, 10},
                { 10, "", 226, 33, 1369, 685, 0, 1, 0, -1, -1, 5, 9, 6},
                { 11, "", 1595, 33, 325, 1047, 0, 1, 0, -1, -1, 1, 0, 8}

            };
            
            if(!allDocks) allDocks = defaultDocks;
            
            for(int i = 0; i < numElements; i++)
            {
                //MPDockData* dockData = &allDocks[i];
                Dock* dock = (Dock*)MemAlloc(sizeof(Dock));
                m_docks.push_back(IM_PLACEMENT_NEW(dock) Dock());
            }
            
            for(int i = 0; i < numElements; i++)
            {
                MPDockData* dockData = &allDocks[i];
                Dock* dock = m_docks[dockData->idx];
                dock->last_frame = 0;
                dock->invalid_frames = 0;
                dock->label = ImStrdup(dockData->label);
                dock->id = ImHash(dockData->label, 0);
                dock->pos.x = dockData->x;
                dock->pos.y = dockData->y;
                dock->size.x = dockData->size_x;
                dock->size.y = dockData->size_y;
                dock->status = (Status_)dockData->status;
                dock->active = dockData->active;
                dock->opened = dockData->opened;
                dock->prev_tab = getDockByIndex(dockData->prev);
                dock->next_tab = getDockByIndex(dockData->next);
                dock->children[0] = getDockByIndex(dockData->child0);
                dock->children[1] = getDockByIndex(dockData->child1);
                dock->parent = getDockByIndex(dockData->parent);
            }
        }
        ...
        void save(std::string const & path)
        {
            ...
            {
                Dock& dock = *m_docks[i];
                std::cout << "dock" << (void*)&dock << " = {\n";
                std::cout << "index = " << i << ",\n";
                std::cout << "label = \"" << dock.label << "\",\n";
                std::cout << "x = " << (int)dock.pos.x << ",\n";
                std::cout << "y = " << (int)dock.pos.y << ",\n";
                std::cout << "size_x = " << (int)dock.size.x << ",\n";
                std::cout << "size_y = " << (int)dock.size.y << ",\n";
                std::cout << "status = " << (int)dock.status << ",\n";
                std::cout << "active = " << (int)dock.active << ",\n";
                std::cout << "opened = " << (int)dock.opened << ",\n";
                std::cout << "prev = " << (int)getDockIndex(dock.prev_tab) << ",\n";
                std::cout << "next = " << (int)getDockIndex(dock.next_tab) << ",\n";
                std::cout << "child0 = " << (int)getDockIndex(dock.children[0]) << ",\n";
                std::cout << "child1 = " << (int)getDockIndex(dock.children[1]) << ",\n";
                std::cout << "parent = " << (int)getDockIndex(dock.parent) << "\n";
                if (i < m_docks.size() - 1)
                    std::cout << "},\n";
                else
                    std::cout << "}\n";
            ...
            unsigned int numElements = m_docks.size();
            MPDockData * allDocks = (MPDockData*)malloc(sizeof(MPDockData) * numElements);
            ...
            for (int i = 0; i < m_docks.size(); ++i)
            {
                ...
                std::cout << "{ " << i << ", ";
                std::cout << "\"" << dock.label << "\", ";
                std::cout << (int)dock.pos.x << ", ";
                std::cout << (int)dock.pos.y << ", ";
                std::cout << (int)dock.size.x << ", ";
                std::cout << (int)dock.size.y << ", ";
                std::cout << (int)dock.status << ", ";
                std::cout << (int)dock.active << ", ";
                std::cout << (int)dock.opened << ", ";
                std::cout << (int)getDockIndex(dock.prev_tab) << ", ";
                std::cout << (int)getDockIndex(dock.next_tab) << ", ";
                std::cout << (int)getDockIndex(dock.children[0]) << ", ";
                std::cout << (int)getDockIndex(dock.children[1]) << ", ";
                std::cout << (int)getDockIndex(dock.parent) << "";
                if (i < m_docks.size() - 1)
                    std::cout << "},\n";
                else
                    std::cout << "}\n";
                ...
                allDocks[i].idx = i;
                strcpy(allDocks[i].label, dock.label);
                allDocks[i].x = dock.pos.x;
                allDocks[i].y = dock.pos.y;
                allDocks[i].size_x = dock.size.x;
                allDocks[i].size_y = dock.size.y;
                allDocks[i].status = dock.status;
                allDocks[i].active = dock.active;
                allDocks[i].opened = dock.opened;
                allDocks[i].prev = getDockIndex(dock.prev_tab);
                allDocks[i].next = getDockIndex(dock.next_tab);
                allDocks[i].child0 = getDockIndex(dock.children[0]);
                allDocks[i].child1 = getDockIndex(dock.children[1]);
                allDocks[i].parent = getDockIndex(dock.parent);
            }

            std::ofstream output( path, std::ios::binary );
            output.write((char*)&numElements, sizeof(unsigned int));
            output.seekp(output.beg + sizeof(unsigned int));
            output.write((char*)allDocks, sizeof(MPDockData) * numElements);
            output.close();
        }
        ...
        void load(std::string const & path)
        {
            ...
            unsigned int numElements = 12;
            
            MPDockData* allDocks = nullptr;
            std::ifstream input( path, std::ios::binary );
            if(input.good())
            {
                input.read((char*)&numElements, sizeof(unsigned int));
                allDocks = (MPDockData*)malloc(sizeof(MPDockData) * numElements);
                input.seekg(input.beg + sizeof(unsigned int));
                input.read((char*)allDocks, sizeof(MPDockData) * numElements);
            }
            
            reset(allDocks, numElements);
    ...
    void SaveDock(std::string const & path)
    {
        g_dock.save(path);
    }
    
    
    void LoadDock(std::string const & path)
    {
        g_dock.load(path);
    }
    ...
    bool IsAnyFloatingDockDragged()
    {
        return g_dock.isAnyFloatingDockDragged();
    }
