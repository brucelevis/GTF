...
void GTFNodeGraph::drawHermite(ImDrawList* drawList, ImVec2 p1, ImVec2 p2, int STEPS = 32)
{
    ImVec2 diff = p2 - p1;
...
bool GTFNodeGraph::isConnectorHovered(GTFNodeConnectionBase* c, ImVec2 offset)
{
	...
	return ((xd * xd) + (yd *yd)) < (GTFNODEGRAPH::GTF_NODE_SLOT_RADIUS * GTFNODEGRAPH::GTF_NODE_SLOT_RADIUS);
}

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

GTFNodeConnectionBase* GTFNodeGraph::getHoverCon(ImVec2 offset, ImVec2* pos, bool resetCon)
{
	for (GTFNode* node : nodeList)
	{
...
GTFNode* GTFNodeGraph::findNodeByCon(GTFNodeConnectionBase const * findCon)
{
	for (GTFNode* node : nodeList)
	{
		for (GTFNodeConnectionBase* con : node->inputConnections)
...
void GTFNodeGraph::renderLines(ImDrawList* drawList, ImVec2 offset)
{
	for (GTFNode* node : nodeList)
	{
		for (GTFNodeConnectionBase* con : node->inputConnections)
...
void GTFNodeGraph::displayNode(ImDrawList* drawList, ImVec2 offset, GTFNode* node, int& node_selected)
{
	...
	ImVec2 pos = node_rect_min + GTFNODEGRAPH::GTF_NODE_WINDOW_PADDING;
	...
	for (GTFNodeConnectionBase* con : node->inputConnections)
	{
		...
		drawList->AddCircleFilled(node_rect_min + ImVec2(con->posX, con->posY), GTFNODEGRAPH::GTF_NODE_SLOT_RADIUS, conColor);
...
void GTFNodeGraph::updateGUI()
{
    ...
    GTFNodeConnectionBase* hoveredCon = getHoverCon(curOffset - scrolling, &hoveredConPos, false);
