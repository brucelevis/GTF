//
//  GTFGUIGraphEditor.cpp
//  gtf
//
//  Created by David Gallardo on 28/05/16.
...
static void setupConnections(std::vector<Connection*>& connections, ConnectionDesc* connectionDescs, bool isInput)
{
...
static Node* createNodeFromType(ImVec2 pos, NodeType* nodeType)
{
	...
	setupConnections(node->inputConnections, nodeType->inputConnections, true);
	setupConnections(node->outputConnections, nodeType->outputConnections, false);
	...
	switch (s_dragState)
	{
		...
		case DragState_Draging:
		{
			...
			if (!ImGui::IsMouseDown(0))
			{
                ...
                if(con->isInput != s_dragNode.con->isInput)
                {
                    if(con->isInput)
                    {
                    ...
                    {
                        con->output.push_back(s_dragNode.con);
                        s_dragNode.con->input = con;
                    }
...
static void displayNode(ImDrawList* drawList, ImVec2 offset, Node* node, int& node_selected)
{
	...
	for (Connection* con : node->outputConnections)
	{
		...
		ImGui::SetCursorScreenPos(textOffset + ImVec2(con->pos.x - (textSize.x + 10.0f), 0));
		ImGui::Text("%s", con->desc.name);
...
void ShowExampleAppCustomNodeGraph(bool* opened)
{
    ...
    if(s_selNode != nullptr && !ImGui::IsMouseClicked(0))
    {
        
        s_nodes.remove(s_selNode);
        s_nodes.push_back(s_selNode);
        
        //auto nodeIt = std::find(s_nodes.begin(), s_nodes.end(), s_selNode);
        //auto endIt = s_nodes.end();
        //endIt--;
        //std::swap(*endIt, *nodeIt);
        s_selNode = nullptr;
    }
